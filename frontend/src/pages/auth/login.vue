    ...
    <div class="relative rounded-lg">
      <div class="px-6 py-6 lg:px-8">
        ...
        <form class="space-y-6 py-10" @submit="checkForm">
          <label for="password" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"> Your Token </label>
          <input
            type="password"
            name="password"
            id="password"
            ...
            :class="errors ? 'border border-red-500 text-red-900 placeholder-red-700 focus:ring-red-500 focus:border-red-500 dark:border-red-400' : ''"
            v-model="this.token"
          />
          ...
          <div class="flex justify-between">
            ...
            <router-link to="/forgot" class="text-sm mt-8 text-[#f5c518] hover:underline dark:text-[#cba724]">Lost Token? </router-link>
          </div>
          <button
            type="submit"
          ...
          <div class="text-sm font-medium text-gray-500 dark:text-gray-300">
            Not registered?
            <router-link to="/register" href="#" class="text-[#f5c518] hover:underline dark:text-[#cba724]"> Create account </router-link>
          </div>
        </form>
      </div>
    </div>
      ...
      await axios({
        ...
        .then((result) => {
          ...
          if (result.data.success) {
            ...
            localStorage.setItem("auth:cinhall", JSON.stringify(result.data.token));
            this.$router.go(-1);
            Swal.fire({
              position: "center",
              icon: "success",
              title: "welcom back <p class='text-red-600'>" + result.data.user + "</p>",
